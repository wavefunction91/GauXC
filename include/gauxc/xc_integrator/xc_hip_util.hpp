...
inline void process_batches_hip_replicated_p( size_t n_deriv, Args&&... args ) {
  if( n_deriv == 0 )
    process_batches_hip_replicated_p<F,0>( std::forward<Args>(args)... );
  else if( n_deriv == 1 )
    process_batches_hip_replicated_p<F,1>( std::forward<Args>(args)... );
  else
