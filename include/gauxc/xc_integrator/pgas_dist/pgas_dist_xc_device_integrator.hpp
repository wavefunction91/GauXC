...
/// Base class for PGASDistributedXCIntegrator implentations on Device execution spaces
template <typename ValueType>
class PGASDistributedXCDeviceIntegrator : public PGASDistributedXCIntegratorImpl<ValueType> {
...
public:
  ...
  PGASDistributedXCDeviceIntegrator( Args&&... args) :
    base_type( std::forward<Args>(args)... ) { }

  virtual ~PGASDistributedXCDeviceIntegrator() noexcept;

};

extern template class PGASDistributedXCDeviceIntegrator<double>;



/// Factory to generate PGASDistributedXCDeviceIntegrator instances
template <typename ValueType>
struct PGASDistributedXCDeviceIntegratorFactory {

  using impl_type = PGASDistributedXCIntegratorImpl<ValueType>;
  using ptr_return_t = std::unique_ptr<impl_type>;

  /** Generate a PGASDistributedXCDeviceIntegrator instance
   *
...
extern template struct PGASDistributedXCDeviceIntegratorFactory<double>;
