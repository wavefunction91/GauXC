...
void AoSScheme1CUTLASSBase::eval_xmat(double fac, XCDeviceData* _data, bool do_grad, density_id den_id ){
  ...
  auto dmat_ptr = static_stack.den_selector(den_id);
  ...
  auto dmat_array = cutlass_stack.dmat_array(den_id);
  cutlass_gemm(
...
void AoSScheme1CUTLASSBase::inc_vxc( XCDeviceData* _data, density_id den_id,  bool do_m){
  ...
  auto static_stack  = data->static_stack;
  auto aos_stack     = data->aos_stack;
  auto vxc_ptr = static_stack.vxc_selector(den_id);
  sym_task_inc_potential( ntasks, aos_stack.device_tasks, vxc_ptr, nbf, 
