...
__global__ void eval_vvars_gga_uks_kernel( size_t ntasks, XCDeviceTask* tasks_device) {
  ...
  auto*     den_pos_eval_device   = task.den_pos;
  auto*     den_pos_x_eval_device = task.dden_posx;
  auto*     den_pos_y_eval_device = task.dden_posy;
  auto*     den_pos_z_eval_device = task.dden_posz;

  auto*     den_neg_eval_device   = task.den_neg;
  auto*     den_neg_x_eval_device = task.dden_negx;
  auto*     den_neg_y_eval_device = task.dden_negy;
  auto*     den_neg_z_eval_device = task.dden_negz;
...
// Since this is for all intents and purposes identical to eval_uvars_gga_rks_kernel, they may be merged together in the future
