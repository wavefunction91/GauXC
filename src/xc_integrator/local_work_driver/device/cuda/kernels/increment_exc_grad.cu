...
__global__ __launch_bounds__(512,1) void increment_exc_grad_mgga_kernel(
  uint32_t                        nshell,
  ShellToTaskDevice* __restrict__ shell_to_task,
  XCDeviceTask*      __restrict__ device_tasks,
  double*            __restrict__ EXC_GRAD
...
void increment_exc_grad_mgga( size_t nshell, bool need_lapl, 
  ...
  increment_exc_grad_mgga_kernel<<<blocks, threads, 0 , stream>>>(
    nshell, shell_to_task, device_tasks, EXC_GRAD 
  );
}
