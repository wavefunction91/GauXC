...
__global__ __launch_bounds__(1024,1) void collocation_device_shell_to_task_kernel_spherical_gradient_0(
  size_t                            ntasks,
  ...
  for( int itask = global_warp_id; itask < ntasks; itask += nwarp_global ) {
    ...
    for( int ipt = warp_rank; ipt < npts; ipt += cuda::warp_size ) {
      ...
      ang_eval = radial_eval;
      dang_eval_x = radial_eval_x;
      dang_eval_y = radial_eval_y;
      dang_eval_z = radial_eval_z;
