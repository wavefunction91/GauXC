...
__global__ __launch_bounds__(1024,1) void collocation_device_shell_to_task_kernel_spherical_gradient_2(
  size_t                            ntasks,
  ...
  for( int itask = global_warp_id; itask < ntasks; itask += nwarp_global ) {
    ...
    for( int ipt = warp_rank; ipt < npts; ipt += cuda::warp_size ) {
      ...
      ang_eval = sqrt_3*radial_eval*x*y;
      dang_eval_x = sqrt_3*y*(radial_eval + radial_eval_x*x);
      dang_eval_y = sqrt_3*x*(radial_eval + radial_eval_y*y);
      dang_eval_z = sqrt_3*radial_eval_z*x*y;
      ...
      ang_eval = sqrt_3*radial_eval*y*z;
      dang_eval_x = sqrt_3*radial_eval_x*y*z;
      dang_eval_y = sqrt_3*z*(radial_eval + radial_eval_y*y);
      dang_eval_z = sqrt_3*y*(radial_eval + radial_eval_z*z);
      ...
      ang_eval = radial_eval*(-x*x - y*y + 2*z*z)/2;
      dang_eval_x = -radial_eval*x - radial_eval_x*(x*x + y*y - 2*z*z)/2;
      dang_eval_y = -radial_eval*y - radial_eval_y*(x*x + y*y - 2*z*z)/2;
      dang_eval_z = 2*radial_eval*z - radial_eval_z*(x*x + y*y - 2*z*z)/2;
      ...
      ang_eval = sqrt_3*radial_eval*x*z;
      dang_eval_x = sqrt_3*z*(radial_eval + radial_eval_x*x);
      dang_eval_y = sqrt_3*radial_eval_y*x*z;
      dang_eval_z = sqrt_3*x*(radial_eval + radial_eval_z*z);
      ...
      ang_eval = sqrt_3*radial_eval*(x*x - y*y)/2;
      dang_eval_x = sqrt_3*(radial_eval*x + radial_eval_x*(x*x - y*y)/2);
      dang_eval_y = sqrt_3*(-radial_eval*y + radial_eval_y*(x*x - y*y)/2);
      dang_eval_z = sqrt_3*radial_eval_z*(x*x - y*y)/2;
