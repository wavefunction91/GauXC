...
dev_integral_0_task_batched(
  ...
  for (int i_block = 0; i_block < npts_block; i_block++) {
    ...
    for (int j = num_warps*blockIdx.y+warpId; j < nsp; j+=num_warps*gridDim.y) {
      ...
      dev_integral_0_task<primpair_shared_limit, points_per_subtask>(
...
void dev_integral_0_dispatcher(dim3 nblock, dim3 nthreads, int max_primpair, cudaStream_t stream, 
  Args&&... args) {
