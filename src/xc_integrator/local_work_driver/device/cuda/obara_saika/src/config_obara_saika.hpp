...
template <int M>
__device__ inline __attribute__((always_inline)) double gauxc_boys_element(double *boys_table, double T) {
  if constexpr (M != 0) {
    if (T < DEFAULT_MAX_T) {
      double* boys_m = (boys_table_ + M * DEFAULT_LD_TABLE * DEFAULT_NSEGMENT);
...
#define BOYS_FUNCTION_IMPLEMENTATION(M)					\
  template __device__ inline __attribute__((always_inline)) double gauxc_boys_element<M>(double *, double); \
