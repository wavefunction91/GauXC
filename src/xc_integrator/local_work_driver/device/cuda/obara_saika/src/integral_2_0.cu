...
#define K20_WARPSIZE 32
#define K20_NUMWARPS 8
#define K20_NUMTHREADS (K20_WARPSIZE * K20_NUMWARPS)
#define K20_USESHARED 1
#define K20_MAX_PRIMPAIRS 32

__inline__ __device__ void dev_integral_2_0_task(
  const int i,
  const int npts,
  const int nprim_pairs,
  // Point data
  double4 (&s_task_data)[K20_NUMTHREADS],
  // Shell Pair Data
  const shell_pair* sp,
  // Output Data
  const double *Xi,
  ...
  const int laneId = threadIdx.x % K20_WARPSIZE;

  const auto& prim_pairs = sp->prim_pairs();

#if K20_USESHARED
  // Load Primpairs to shared
  __shared__ GauXC::PrimitivePair<double> s_prim_pairs[K20_NUMWARPS][K20_MAX_PRIMPAIRS];

  const int warpId = (threadIdx.x / K20_WARPSIZE);
  const int32_t* src = (int32_t*) &(prim_pairs[0]);
  int32_t* dst = (int32_t*) &(s_prim_pairs[warpId][0]);

  for (int i = laneId; i < nprim_pairs * sizeof(GauXC::PrimitivePair<double>) / sizeof(int32_t); i+=K20_WARPSIZE) {
    dst[i] = src[i]; 
  }
  __syncwarp();
#endif
  ...
  for (int i = 0; i < K20_NUMTHREADS / K20_WARPSIZE; i++) {
    ...
    const int pointIndex = i * K20_WARPSIZE + laneId;
...
template <bool swap>
__global__ void 
__launch_bounds__(K20_NUMTHREADS, 1)
dev_integral_2_0_task_batched(
  ...
  __shared__ double4 s_task_data[K20_NUMTHREADS];
  ...
  for (int i_block = 0; i_block < npts_block; i_block++) {
    ...
    for (int j = K20_NUMWARPS*blockIdx.y+warpId; j < nsp; j+=K20_NUMWARPS*gridDim.y) {
  ...
  void integral_2_0_task_batched(
    ...
    if (swap) {
      dev_integral_2_0_task_batched<true><<<nblocks, nthreads, 0, stream>>>(
        ntasks, nsubtask, 
        device_tasks, task2sp, 
        (int4*) subtasks, nprim_pairs_device, sp_ptr_device,
        boys_table );
    } else {
      dev_integral_2_0_task_batched<false><<<nblocks, nthreads, 0, stream>>>(
        ntasks, nsubtask, 
        device_tasks, task2sp, 
        (int4*) subtasks, nprim_pairs_device, sp_ptr_device,
        boys_table );
