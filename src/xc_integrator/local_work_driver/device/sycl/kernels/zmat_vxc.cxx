...
void zmat_lda_vxc_rks_kernel( size_t ntasks,
  ...
  const int tid_x = it.get_global_id(2);
  const int tid_y = it.get_global_id(1);
...
void zmat_lda_vxc_uks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
template<density_id den_selector>
void zmat_lda_vxc_gks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
void zmat_gga_vxc_rks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
template<density_id den_selector>
void zmat_gga_vxc_uks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
template<density_id den_selector>
void zmat_gga_vxc_gks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
void zmat_mgga_vxc_rks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
void zmat_mgga_vxc_uks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
#define ZMAT_VXC_KERN(xc_approx) \
  ...
  sycl::launch_dim blocks( util::div_ceil( max_npts, threads.x ), \
               util::div_ceil( max_nbf,  threads.y ), \
               ntasks ); \
  switch( scheme ) { \
    case RKS: \
      sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_vxc_rks_kernel", \
        [=](){ zmat_##xc_approx##_vxc_rks_kernel( ntasks, tasks_device ); } ); \
      break; \
    case UKS: \
      if ( sel == DEN_S )       sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_vxc_uks_kernel", \
        [=](){ zmat_##xc_approx##_vxc_uks_kernel<DEN_S>( ntasks, tasks_device ); } ); \
      else if ( sel == DEN_Z )  sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_vxc_uks_kernel", \
        [=](){ zmat_##xc_approx##_vxc_uks_kernel<DEN_Z>( ntasks, tasks_device ); } ); \
      else GAUXC_GENERIC_EXCEPTION( "zmat_##xc_approx##_vxc invalid density" ); \
      break; \
    case GKS: \
      if ( sel == DEN_S )       sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_vxc_gks_kernel", \
        [=](){ zmat_##xc_approx##_vxc_gks_kernel<DEN_S>( ntasks, tasks_device ); } ); \
      else if ( sel == DEN_Z )  sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_vxc_gks_kernel", \
        [=](){ zmat_##xc_approx##_vxc_gks_kernel<DEN_Z>( ntasks, tasks_device ); } ); \
      else if ( sel == DEN_Y )  sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_vxc_gks_kernel", \
        [=](){ zmat_##xc_approx##_vxc_gks_kernel<DEN_Y>( ntasks, tasks_device ); } ); \
      else if ( sel == DEN_X )  sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_vxc_gks_kernel", \
        [=](){ zmat_##xc_approx##_vxc_gks_kernel<DEN_X>( ntasks, tasks_device ); } ); \
      else GAUXC_GENERIC_EXCEPTION( "zmat_##xc_approx##_vxc invalid density" ); \
      break; \
    default: \
...
void zmat_mgga_vxc( size_t            ntasks,
  ...
  if(scheme == RKS) {
    if(do_lapl)
      sycl::launch_kernel( stream, blocks, threads, "zmat_mgga_vxc_rks_kernel",
        [=](){ zmat_mgga_vxc_rks_kernel<true>( ntasks, tasks_device ); } );
    else
      sycl::launch_kernel( stream, blocks, threads, "zmat_mgga_vxc_rks_kernel",
        [=](){ zmat_mgga_vxc_rks_kernel<false>( ntasks, tasks_device ); } );
  } else if(scheme == UKS) {
    switch(sel) {
      case DEN_S:
        if(do_lapl)
          sycl::launch_kernel( stream, blocks, threads, "zmat_mgga_vxc_uks_kernel",
            [=](){ zmat_mgga_vxc_uks_kernel<true, DEN_S>( ntasks, tasks_device ); } );
        else
          sycl::launch_kernel( stream, blocks, threads, "zmat_mgga_vxc_uks_kernel",
            [=](){ zmat_mgga_vxc_uks_kernel<false, DEN_S>( ntasks, tasks_device ); } );
        break;
      case DEN_Z:
        if(do_lapl)
          sycl::launch_kernel( stream, blocks, threads, "zmat_mgga_vxc_uks_kernel",
            [=](){ zmat_mgga_vxc_uks_kernel<true, DEN_Z>( ntasks, tasks_device ); } );
        else
          sycl::launch_kernel( stream, blocks, threads, "zmat_mgga_vxc_uks_kernel",
            [=](){ zmat_mgga_vxc_uks_kernel<false, DEN_Z>( ntasks, tasks_device ); } );
        break;
    }
  } else {
...
template <bool need_lapl>
void mmat_mgga_vxc_rks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
void mmat_mgga_vxc_uks_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
//void print_zmat_stats( size_t            ntasks,
//                    XCDeviceTask*     tasks_device) {
//
...
void mmat_mgga_vxc( size_t            ntasks,
  ...
  if(scheme == RKS) {
    if(do_lapl)
      sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_vxc_rks_kernel",
        [=](){ mmat_mgga_vxc_rks_kernel<true>( ntasks, tasks_device ); } );
    else
      sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_vxc_rks_kernel",
        [=](){ mmat_mgga_vxc_rks_kernel<false>( ntasks, tasks_device ); } );
  } else if(scheme == UKS) {
    switch(sel) {
      case DEN_S:
        if(do_lapl)
          sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_vxc_uks_kernel",
            [=](){ mmat_mgga_vxc_uks_kernel<true, DEN_S>( ntasks, tasks_device ); } );
        else
          sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_vxc_uks_kernel",
            [=](){ mmat_mgga_vxc_uks_kernel<false, DEN_S>( ntasks, tasks_device ); } );
        break;
      case DEN_Z:
        if(do_lapl)
          sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_vxc_uks_kernel",
            [=](){ mmat_mgga_vxc_uks_kernel<true, DEN_Z>( ntasks, tasks_device ); } );
        else
          sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_vxc_uks_kernel",
            [=](){ mmat_mgga_vxc_uks_kernel<false, DEN_Z>( ntasks, tasks_device ); } );
        break;
    }
  } else {
    GAUXC_GENERIC_EXCEPTION("MGGA + DEVICE + GKS NYI");
  ...
  //print_zmat_stats(ntasks,tasks_device);
}
