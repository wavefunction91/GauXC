...
#include "device/sycl/kernels/collocation/collocation_angular_cartesian.hpp"
#include "device/sycl/kernels/collocation/collocation_angular_spherical_unnorm.hpp"
...
void collocation_device_masked_combined_kernel(
  ...
  const size_t tid_x = it.get_global_id(2);
  const size_t tid_y = it.get_global_id(1);

  if( it.get_group(0) < ntasks ) {

    auto& task = device_tasks[ it.get_group(0) ];
