...
inline void modify_weights_ssf_kernel_1d(
        size_t                            npts,
        size_t                            natoms,
  const double*                           RAB,
        int32_t                           ldRAB,
  ...
  auto sFrisch = [&] (double x) {
    if( ::sycl::fabs(x) < integrator::magic_ssf_factor<> ) return (0.5 - (0.5/16.) * gFrisch(x));
  ...
  for( size_t ipt = tid_x; ipt < npts; ipt += nt_x ) {
...
void partition_weights_ssf_1d( int32_t npts, int32_t natoms, const double* RAB,
  ...
  const size_t threads = sycl::max_threads_per_thread_block;
  const size_t blocks  = util::div_ceil( npts, threads );

  GAUXC_SYCL_ERROR( "SSF 1D Weights Launch Failed",
    stream.parallel_for( ::sycl::nd_range<1>( blocks*threads, threads ),
      [=](::sycl::nd_item<1>)
      [[sycl::reqd_sub_group_size(GauXC::sycl::warp_size)]] {
        modify_weights_ssf_kernel_1d( npts, natoms, RAB, ldRAB, coords,
          dist, lddist, iparent, dist_nearest, weights );
      })
  );

}

inline void eval_weight_1st_deriv_contracted_ssf_kernel_1d(
        size_t                            npts,
        size_t                            natoms,
  const double*                           RAB,
        int32_t                           ldRAB,
  ...
  for( size_t ipt = tid_x; ipt < npts; ipt += nt_x ) {
    ...
    if (::sycl::fabs(w_times_f_i) < w_times_f_thresh) continue; // weight derivative = 0 when p_A = 0
    const auto iParent = iparent_device[ipt];
    ...
    if( iParent != iB ) 
    {
      ...
      if( ::sycl::fabs(mu_AB) < safe_magic_ssf_bound) {
      ...
      if( PB >  weight_tol ) 
        for( int iC = 0; iC < natoms; iC++ ) {
          ...
          if(::sycl::fabs(mu_BC) < safe_magic_ssf_bound){
            ...
            if(iC != iParent) {
              ...
              atomic_add_device(exc_grad_w_device + 3*iC + 0, C_x);
              atomic_add_device(exc_grad_w_device + 3*iC + 1, C_y);
              atomic_add_device(exc_grad_w_device + 3*iC + 2, C_z);

              // Update parent atom
              atomic_add_device(exc_grad_w_device + 3*iParent + 0, -C_x);
              atomic_add_device(exc_grad_w_device + 3*iParent + 1, -C_y);
              atomic_add_device(exc_grad_w_device + 3*iParent + 2, -C_z);
            }
          }
        }

        atomic_add_device(exc_grad_w_device + 3*iB + 0, exc_grad_w_iBx);
        atomic_add_device(exc_grad_w_device + 3*iB + 1, exc_grad_w_iBy);
        atomic_add_device(exc_grad_w_device + 3*iB + 2, exc_grad_w_iBz);

        // Update parent atom
        atomic_add_device(exc_grad_w_device + 3*iParent + 0, -exc_grad_w_iBx);
        atomic_add_device(exc_grad_w_device + 3*iParent + 1, -exc_grad_w_iBy);
        atomic_add_device(exc_grad_w_device + 3*iParent + 2, -exc_grad_w_iBz);
...
void eval_weight_1st_deriv_contracted_ssf_1d( int32_t npts, int32_t natoms, const double* RAB,
  ...
  const size_t threads = sycl::max_threads_per_thread_block/4;
  const size_t blocks  = util::div_ceil( npts, threads );

  GAUXC_SYCL_ERROR( "SSF 1D Weight Derivative Launch Failed",
    stream.parallel_for( ::sycl::nd_range<1>( blocks*threads, threads ),
      [=](::sycl::nd_item<1>)
      [[sycl::reqd_sub_group_size(GauXC::sycl::warp_size)]] {
        eval_weight_1st_deriv_contracted_ssf_kernel_1d( npts, natoms, RAB,
          ldRAB, coords, points_x, points_y, points_z, dist, lddist, iparent,
          dist_nearest, w_times_f, exc_grad_w );
      })
  );
