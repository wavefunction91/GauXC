...
void zmat_lda_fxc_kernel( size_t ntasks,
...
template<density_id den_selector>
void zmat_gga_fxc_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
#define ZMAT_FXC_KERN(xc_approx) \
  ...
  sycl::launch_dim blocks( util::div_ceil( max_npts, threads.x ), \
               ...
               ntasks ); \
  if ( sel == DEN_S )       sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_fxc_kernel", \
    [=](){ zmat_##xc_approx##_fxc_kernel<DEN_S>( ntasks, tasks_device ); } ); \
  else if ( sel == DEN_Z )  sycl::launch_kernel( stream, blocks, threads, "zmat_" #xc_approx "_fxc_kernel", \
    [=](){ zmat_##xc_approx##_fxc_kernel<DEN_Z>( ntasks, tasks_device ); } ); \
...
void zmat_mgga_fxc( size_t            ntasks,
  ...
  switch(sel) {
    case DEN_S:
        sycl::launch_kernel( stream, blocks, threads, "zmat_gga_fxc_kernel",
          [=](){ zmat_gga_fxc_kernel<DEN_S>( ntasks, tasks_device ); } );
      break;
    case DEN_Z:
        sycl::launch_kernel( stream, blocks, threads, "zmat_gga_fxc_kernel",
          [=](){ zmat_gga_fxc_kernel<DEN_Z>( ntasks, tasks_device ); } );
      break;
  }
...
void mmat_mgga_fxc_kernel( size_t ntasks,
                                     XCDeviceTask* tasks_device ) {
...
void mmat_mgga_fxc( size_t            ntasks,
  ...
  switch(sel) {
    case DEN_S:
        sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_fxc_kernel",
          [=](){ mmat_mgga_fxc_kernel<DEN_S>( ntasks, tasks_device ); } );
      break;
    case DEN_Z:
        sycl::launch_kernel( stream, blocks, threads, "mmat_mgga_fxc_kernel",
          [=](){ mmat_mgga_fxc_kernel<DEN_Z>( ntasks, tasks_device ); } );
      break;
  }
