...
void increment_exc_grad_lda_rks_kernel(
  uint32_t                        nshell,
  ...
  for( uint32_t ish = it.get_group(0); ish < nshell; ish += it.get_group_range(0) ) {
    ...
    const int global_warp_id =
      it.get_global_id(2) / sycl::warp_size;
    ...
    for( uint32_t itask = global_warp_id; itask < ntasks; itask += nwarp_global ) {
      ...
      for( uint32_t ipt = it.get_local_id(2) % sycl::warp_size;
           ipt < npts;
           ipt += sycl::warp_size ) {
      ...
      if constexpr( with_weight_derivatives ) {
        atomic_add_device( EXC_GRAD + 3*iParent + 0, 2.0 * g_acc_x_task );
        atomic_add_device( EXC_GRAD + 3*iParent + 1, 2.0 * g_acc_y_task );
        atomic_add_device( EXC_GRAD + 3*iParent + 2, 2.0 * g_acc_z_task );
      }

    } // Loop over tasks assigned to shell

    constexpr auto warp_size = sycl::warp_size;
    g_acc_x = -2. * sycl::warp_reduce_sum<warp_size>( g_acc_x );
    g_acc_y = -2. * sycl::warp_reduce_sum<warp_size>( g_acc_y );
    g_acc_z = -2. * sycl::warp_reduce_sum<warp_size>( g_acc_z );

    if( (it.get_local_id(2) % sycl::warp_size) == 0 ) {

      atomic_add_device( EXC_GRAD + 3*iCen + 0, g_acc_x );
      atomic_add_device( EXC_GRAD + 3*iCen + 1, g_acc_y );
      atomic_add_device( EXC_GRAD + 3*iCen + 2, g_acc_z );
    }
...
template<bool with_weight_derivatives>
void increment_exc_grad_lda_uks_kernel(
  uint32_t                        nshell,
  ShellToTaskDevice* __restrict__ shell_to_task,
  XCDeviceTask*      __restrict__ device_tasks,
  double*            __restrict__ EXC_GRAD
...
void increment_exc_grad_lda( integrator_ks_scheme ks_scheme, size_t nshell, ShellToTaskDevice* shell_to_task,
  ...
  sycl::launch_dim threads(1024), blocks(1,1,nshell);

  switch(ks_scheme) {
    case RKS:
      if (with_weight_derivatives) {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_lda_rks_kernel<true>",
          [=](){
            increment_exc_grad_lda_rks_kernel<true>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      } else {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_lda_rks_kernel<false>",
          [=](){
            increment_exc_grad_lda_rks_kernel<false>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      }
      break;
    case UKS:
      if (with_weight_derivatives) {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_lda_uks_kernel<true>",
          [=](){
            increment_exc_grad_lda_uks_kernel<true>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      } else {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_lda_uks_kernel<false>",
          [=](){
            increment_exc_grad_lda_uks_kernel<false>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      }
      break;
...
template<bool with_weight_derivatives>
void increment_exc_grad_gga_rks_kernel(
  uint32_t                        nshell,
  ShellToTaskDevice* __restrict__ shell_to_task,
  XCDeviceTask*      __restrict__ device_tasks,
  double*            __restrict__ EXC_GRAD
...
template<bool with_weight_derivatives>
void increment_exc_grad_gga_uks_kernel(
  uint32_t                        nshell,
  ShellToTaskDevice* __restrict__ shell_to_task,
  XCDeviceTask*      __restrict__ device_tasks,
  double*            __restrict__ EXC_GRAD
...
void increment_exc_grad_gga( integrator_ks_scheme ks_scheme, size_t nshell, ShellToTaskDevice* shell_to_task,
  ...
  ::sycl::queue& stream = queue.queue_as<util::sycl_queue>();
  sycl::launch_dim threads(512), blocks(1,1,nshell);

  switch(ks_scheme) {
    case RKS:
      if (with_weight_derivatives) {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_gga_rks_kernel<true>",
          [=](){
            increment_exc_grad_gga_rks_kernel<true>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      } else {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_gga_rks_kernel<false>",
          [=](){
            increment_exc_grad_gga_rks_kernel<false>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      }
      break;
    case UKS:
      if (with_weight_derivatives) {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_gga_uks_kernel<true>",
          [=](){
            increment_exc_grad_gga_uks_kernel<true>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      } else {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_gga_uks_kernel<false>",
          [=](){
            increment_exc_grad_gga_uks_kernel<false>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      }
      break;
...
template<bool with_weight_derivatives>
void increment_exc_grad_mgga_rks_kernel(
  uint32_t                        nshell,
  ShellToTaskDevice* __restrict__ shell_to_task,
  XCDeviceTask*      __restrict__ device_tasks,
  double*            __restrict__ EXC_GRAD
...
template<bool with_weight_derivatives>
void increment_exc_grad_mgga_uks_kernel(
  uint32_t                        nshell,
  ShellToTaskDevice* __restrict__ shell_to_task,
  XCDeviceTask*      __restrict__ device_tasks,
  double*            __restrict__ EXC_GRAD
...
void increment_exc_grad_mgga( integrator_ks_scheme ks_scheme, size_t nshell, bool need_lapl,
  ...
  if(need_lapl) GAUXC_GENERIC_EXCEPTION("SYCL + MGGA/LAPL EXC GRAD NYI");
  ...
  switch(ks_scheme) {
    case RKS:
      if (with_weight_derivatives) {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_mgga_rks_kernel<true>",
          [=](){
            increment_exc_grad_mgga_rks_kernel<true>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      } else {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_mgga_rks_kernel<false>",
          [=](){
            increment_exc_grad_mgga_rks_kernel<false>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      }
      break;
    case UKS:
      if (with_weight_derivatives) {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_mgga_uks_kernel<true>",
          [=](){
            increment_exc_grad_mgga_uks_kernel<true>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      } else {
        sycl::launch_kernel( stream, blocks, threads, "increment_exc_grad_mgga_uks_kernel<false>",
          [=](){
            increment_exc_grad_mgga_uks_kernel<false>(
              nshell, shell_to_task, device_tasks, EXC_GRAD );
          });
      }
      break;
