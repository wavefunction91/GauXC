...
void eval_vvar_lda_kern( size_t        ntasks,
  ...
  if( it.get_local_id(2) == 0 and tid_y < npts ) {
    atomic_add_device( den_eval_device   + tid_y, den_reg );
  }

}

inline void eval_uvars_lda_rks_kernel(size_t ntasks, XCDeviceTask* tasks_device) {
  // eval_vvars populated uvar storage already in the case of LDA+RKS
  return;
}
inline void eval_tmat_lda_rks_kernel(size_t ntasks, XCDeviceTask* tasks_device) {
...
inline void eval_uvars_lda_uks_kernel(size_t        ntasks,
  XCDeviceTask* tasks_device ) {
...
inline void eval_tmat_lda_uks_kernel(size_t        ntasks,
  XCDeviceTask* tasks_device ) {
...
inline void eval_uvars_lda_gks_kernel(size_t        ntasks,
                                           XCDeviceTask* tasks_device ) {
