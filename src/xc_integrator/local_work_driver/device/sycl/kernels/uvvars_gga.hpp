...
void eval_vvar_gga_kern( size_t        ntasks,
  ...
  auto& den_shared = GauXC::sycl::local_mem<double[4][warp_size][VVAR_KERNEL_SM_BLOCK+1]>();
  ...
  for ( int bid_x = it.get_group(2) * blockDim_x;
    ...
    for ( int bid_y = it.get_group(1) * VVAR_KERNEL_SM_BLOCK;
          bid_y < npts;
          bid_y += VVAR_KERNEL_SM_BLOCK * it.get_group_range(1) ) {

      for (int sm_y = threadIdx_y; sm_y < VVAR_KERNEL_SM_BLOCK; sm_y += blockDim_y) {
        ...
        if (bid_y + threadIdx_x < npts and bid_x + sm_y < nbf) {
          ...
          den_shared[0][threadIdx_x][sm_y] = bf_col  [ bid_y + threadIdx_x ] * db_col[ bid_y + threadIdx_x ];
          den_shared[1][threadIdx_x][sm_y] = bf_x_col[ bid_y + threadIdx_x ] * db_col[ bid_y + threadIdx_x ];
          den_shared[2][threadIdx_x][sm_y] = bf_y_col[ bid_y + threadIdx_x ] * db_col[ bid_y + threadIdx_x ];
          den_shared[3][threadIdx_x][sm_y] = bf_z_col[ bid_y + threadIdx_x ] * db_col[ bid_y + threadIdx_x ];
        }
      }
      ...
      for (int sm_y = threadIdx_y; sm_y < VVAR_KERNEL_SM_BLOCK; sm_y += blockDim_y) {
        ...
        double den_reg = den_shared[0][sm_y][threadIdx_x];
        double dx_reg  = den_shared[1][sm_y][threadIdx_x];
        double dy_reg  = den_shared[2][sm_y][threadIdx_x];
        double dz_reg  = den_shared[3][sm_y][threadIdx_x];

        // Warp blocks are stored col major
        den_reg =     sycl::warp_reduce_sum<warp_size>( den_reg );
        dx_reg  = 2. * sycl::warp_reduce_sum<warp_size>( dx_reg );
        dy_reg  = 2. * sycl::warp_reduce_sum<warp_size>( dy_reg );
        dz_reg  = 2. * sycl::warp_reduce_sum<warp_size>( dz_reg );


        if( threadIdx_x == 0 and tid_y < npts ) {
          atomic_add_device( den_eval_device   + tid_y, den_reg );
          atomic_add_device( den_x_eval_device + tid_y, dx_reg  );
          atomic_add_device( den_y_eval_device + tid_y, dy_reg  );
          atomic_add_device( den_z_eval_device + tid_y, dz_reg  );
        }
      }
      ::sycl::group_barrier( it.get_group() );
    }
...
inline void eval_uvars_gga_rks_kernel( size_t ntasks, XCDeviceTask* tasks_device) {
...
inline void eval_tmat_gga_rks_kernel( size_t ntasks, XCDeviceTask* tasks_device) {
...
inline void eval_uvars_gga_uks_kernel( size_t ntasks, XCDeviceTask* tasks_device) {
...
inline void eval_tmat_gga_uks_kernel( size_t ntasks, XCDeviceTask* tasks_device) {
...
inline void eval_uvars_gga_gks_kernel( size_t ntasks, XCDeviceTask* tasks_device) {
  ...
  if( tid < npts ) {
    ...
    const auto inv_sqsum2 =
        ::sycl::rsqrt(dels_dot_delz * dels_dot_delz + dels_dot_delx * dels_dot_delx +
             dels_dot_dely * dels_dot_dely);
    const auto sqsum2 = 1./inv_sqsum2;

    double sign = 1.;
    if( ::sycl::signbit(s_sum))
      sign = -1.;


    if (mtemp > dtolsq) {
      const double inv_mnorm = ::sycl::rsqrt(mtemp);
      mnorm = 1./inv_mnorm;
