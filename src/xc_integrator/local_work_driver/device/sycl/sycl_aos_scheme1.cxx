...
std::unique_ptr<XCDeviceData> SyclAoSScheme1<Base>::create_device_data(const DeviceRuntimeEnvironment& rt) {
  return std::make_unique<Data>(rt);
}

template <typename Base> 
void SyclAoSScheme1<Base>::partition_weights( XCDeviceData* _data ) {
  auto* data = dynamic_cast<Data*>(_data);
  if( !data ) GAUXC_BAD_LWD_DATA_CAST();

  auto device_backend = dynamic_cast<SYCLBackend*>(data->device_backend_);
  ...
  sycl_aos_scheme1_weights_wrapper( data->total_npts_task_batch, data->global_dims.natoms,
    ...
    device_backend->master_stream->queue );
}

template <typename Base> 
void SyclAoSScheme1<Base>::eval_weight_1st_deriv_contracted( XCDeviceData* _data, XCWeightAlg alg ) {
  if( alg != XCWeightAlg::SSF ) {
    GAUXC_GENERIC_EXCEPTION("Weight Algorithm NYI for SYCL AoS Scheme1");
  }
  auto* data = dynamic_cast<Data*>(_data);
  if( !data ) GAUXC_BAD_LWD_DATA_CAST();
  ...
  auto static_stack  = data->static_stack;
  auto scheme1_stack = data->scheme1_stack;
  sycl_aos_scheme1_weight_1st_deriv_wrapper( data->total_npts_task_batch, data->global_dims.natoms,
    base_stack.points_x_device, base_stack.points_y_device, base_stack.points_z_device,
    static_stack.rab_device, ldatoms, static_stack.coords_device, 
    scheme1_stack.dist_scratch_device, ldatoms, scheme1_stack.iparent_device, 
    scheme1_stack.dist_nearest_device, base_stack.eps_eval_device,
    static_stack.exc_grad_device, device_backend->master_stream->queue );
}


template struct SyclAoSScheme1<AoSScheme1Base>;
template struct SyclAoSScheme1<AoSScheme1OneMKLBase>;
