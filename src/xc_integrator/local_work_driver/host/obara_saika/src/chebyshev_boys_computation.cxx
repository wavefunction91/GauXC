...
namespace GauXC {
  ...
  void gauxc_boys_init() {
    boys_table_.resize(DEFAULT_LD_TABLE * DEFAULT_NSEGMENT * (DEFAULT_MAX_M + 1));
    generate_boys_table(DEFAULT_NCHEB, DEFAULT_MAX_M, DEFAULT_MAX_T, DEFAULT_NSEGMENT, boys_table_.data(), DEFAULT_LD_TABLE);
  }
  ...
  inline void monomial_expand(size_t npts, const double* coeff, const double *x, double a, double b, double* eval) {
  ...
  template <int M>
  inline void boys_asymp_elements( size_t npts, const double* X, double* FmX ) {
...
#define BOYS_FUNCTION_IMPLEMENTATION(M)					\
  template double gauxc_boys_element<M>(double);			\
  template void gauxc_boys_elements<M>(size_t, double*, double*);
