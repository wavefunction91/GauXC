...
void integral_4_0(size_t npts,
                  ...
                  int ldG, 
                  double *weights) {
   double temp[15 * NPTS_LOCAL];
   double FmT [5 * NPTS_LOCAL];
   ...
   for(size_t p_outer = 0; p_outer < npts; p_outer += NPTS_LOCAL) {
      ...
      for(int i = 0; i < 15 * NPTS_LOCAL; ++i) temp[i] = 0.0;
      ...
      for(int p_inner = 0; p_inner < npts_inner; ++p_inner) {
         ...
         *(Gjk + 0 * ldG) += *(Xik + 9 * ldX) * t9;
         t10 = *(temp + 10 * NPTS_LOCAL + p_inner) * const_value;
         *(Gik + 10 * ldG) += *(Xjk + 0 * ldX) * t10;
         *(Gjk + 0 * ldG) += *(Xik + 10 * ldX) * t10;
         t11 = *(temp + 11 * NPTS_LOCAL + p_inner) * const_value;
         *(Gik + 11 * ldG) += *(Xjk + 0 * ldX) * t11;
         *(Gjk + 0 * ldG) += *(Xik + 11 * ldX) * t11;
         t12 = *(temp + 12 * NPTS_LOCAL + p_inner) * const_value;
         *(Gik + 12 * ldG) += *(Xjk + 0 * ldX) * t12;
         *(Gjk + 0 * ldG) += *(Xik + 12 * ldX) * t12;
         t13 = *(temp + 13 * NPTS_LOCAL + p_inner) * const_value;
         *(Gik + 13 * ldG) += *(Xjk + 0 * ldX) * t13;
         *(Gjk + 0 * ldG) += *(Xik + 13 * ldX) * t13;
         t14 = *(temp + 14 * NPTS_LOCAL + p_inner) * const_value;
