 ...
 */
#include "replicated_sycl_load_balancer.hpp"
...
struct CollisionDetectionSyclData {
    // Inputs
...
std::vector< XCTask > DeviceReplicatedLoadBalancer::create_local_tasks_() const  {
  ...
  data.low_points_device   = util::sycl_malloc<double>(max_nbatches * 3, master_stream);
  data.high_points_device  = util::sycl_malloc<double>(max_nbatches * 3, master_stream);
  data.collisions_device   = util::sycl_malloc<int32_t>(LD_bit * max_nbatches, master_stream);
  data.nbe_list_device     = util::sycl_malloc<size_t>(max_nbatches, master_stream);
  data.counts_device       = util::sycl_malloc<int32_t>(max_nbatches, master_stream);

  data.centers_device      = util::sycl_malloc<double>(nspheres * 3, master_stream);
  data.radii_device        = util::sycl_malloc<double>(nspheres, master_stream);
  data.shell_sizes_device  = util::sycl_malloc<size_t>(nspheres, master_stream);
  ...
  util::sycl_copy(nspheres * 3, data.centers_device, centers[0].data(), master_stream, "Centers HtoD");
  util::sycl_copy(nspheres, data.radii_device, radii.data(), master_stream, "Radii HtoD");
  util::sycl_copy(nspheres, data.shell_sizes_device, shell_sizes.data(), master_stream, "ShellSize HtoD");
  ...
  for (size_t atom_batch = 0; atom_batch < num_atom_batch; ++atom_batch) {
    ...
    util::sycl_copy(ncubes * 3, data.low_points_device, low_points[0].data(), master_stream, "Low points HtoD");
    util::sycl_copy(ncubes * 3, data.high_points_device, high_points[0].data(), master_stream, "High points HtoD");
    ...
    util::sycl_copy(1, &total_collisions, data.counts_device + ncubes - 1, master_stream, "Total collisions DtoH");
    data.position_list_device = util::sycl_malloc<int32_t>(total_collisions, master_stream);
    ...
    util::sycl_device_sync(master_stream);
    // Copy results back to host
    util::sycl_copy(total_collisions, position_list.data(), data.position_list_device, master_stream, "Position List DtoH");
    util::sycl_copy(ncubes, pos_list_idx.data(), data.counts_device, master_stream, "Position List Idx DtoH");
    util::sycl_copy(ncubes, nbe_vec.data(), data.nbe_list_device, master_stream, "NBE counts DtoH");
    util::sycl_free(master_stream, data.position_list_device);
  ...
  util::sycl_free(master_stream, data.low_points_device);
  util::sycl_free(master_stream, data.high_points_device);
  util::sycl_free(master_stream, data.centers_device);
  util::sycl_free(master_stream, data.radii_device);
  util::sycl_free(master_stream, data.shell_sizes_device);
  util::sycl_free(master_stream, data.collisions_device);
  util::sycl_free(master_stream, data.nbe_list_device);
  util::sycl_free(master_stream, data.counts_device);
  util::sycl_free(master_stream, data.temp_storage_device);
