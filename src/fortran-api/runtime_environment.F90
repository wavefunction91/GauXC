...
module gauxc_runtime_environment
...
#ifdef GAUXC_HAS_MPI
  use mpi_f08, only : MPI_Comm, MPI_Comm_f2c, MPI_COMM_WORLD
#endif
  implicit none
  private
  ...
  interface
    function gauxc_runtime_environment_new_c &
      ...
      & (status) &
#endif
      & result(rt) bind(c, name="gauxc_runtime_environment_new")
  ...
  interface
...
#endif
      & result(rt) bind(c, name="gauxc_device_runtime_environment_new")
    ...
    function gauxc_device_runtime_environment_new_mem_c &
      ...
      & (status, mem, mem_sz) &
#endif
      & result(rt) bind(c, name="gauxc_device_runtime_environment_new_mem")
...
contains
  ...
  function gauxc_runtime_environment_new(status) result(rt)
    ...
    type(MPI_Comm) :: comm
    comm = MPI_COMM_WORLD
    rt = gauxc_runtime_environment_new_c(status, MPI_Comm_f2c(comm%mpi_val))
#else
  ...
  function gauxc_runtime_environment_new_mpi(status, comm) result(rt)
    ...
    rt = gauxc_runtime_environment_new_c(status, MPI_Comm_f2c(comm))
  ...
  function gauxc_device_runtime_environment_new &
    ...
    type(MPI_Comm) :: comm
    comm = MPI_COMM_WORLD
    rt = gauxc_device_runtime_environment_new_c(status, MPI_Comm_f2c(comm%mpi_val), fill_fraction)
#else
  ...
  function gauxc_device_runtime_environment_new_mpi &
    ...
    rt = gauxc_device_runtime_environment_new_c(status, MPI_Comm_f2c(comm), fill_fraction)
  ...
  function gauxc_device_runtime_environment_new_mem &
    ...
    type(MPI_Comm) :: comm
    comm = MPI_COMM_WORLD
    rt = gauxc_device_runtime_environment_new_mem_c(status, MPI_Comm_f2c(comm%mpi_val), mem, mem_sz)
#else
  ...
  function gauxc_device_runtime_environment_new_mem_mpi &
    ...
    rt = gauxc_device_runtime_environment_new_mem_c(status, MPI_Comm_f2c(comm), mem, mem_sz)
