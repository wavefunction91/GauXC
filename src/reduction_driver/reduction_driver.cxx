...
void ReductionDriver::allreduce_typeerased( const void* src, void* dest, size_t size, ReductionOp op, std::type_index idx ) {
  if( not pimpl_ ) throw std::runtime_error("Not Initialized");
  pimpl_->allreduce_typeerased(src, dest, size, op, idx);
}

void ReductionDriver::allreduce_inplace_typeerased( void* data, size_t size, ReductionOp op, std::type_index idx ) {
  if( not pimpl_ ) throw std::runtime_error("Not Initialized");
  pimpl_->allreduce_inplace_typeerased(data, size, op, idx);
}
